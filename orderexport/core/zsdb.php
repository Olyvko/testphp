...
class zsDb
{
     ...
     * @var bool
     */
    private static $_blNewConn = false;
    /**
     * Database type
     *
     * @var string
    ...
    /**
     * Sets configs object with method getVar() and properties needed for successful connection for e commerce.
     *
     * @param object $oConfig configs.
     */
    public static function setConfigECommerce($oConfig)
    {
        self::$_dbType = $oConfig->getVar('dbType');
    ...
    public static function getInstance()
    {
        ...
        if (!self::$_instance instanceof zsDb) {

            //do not use simple oxNew here as it goes to eternal cycle
            self::$_instance = new zsDb();
     ...
     * @return zsDb
     */
    public static function setNewConnection()
    {
        self::$_blNewConn = true;
    }
    /**
     ...
     * @return oxLegacyDb
     */
    public static function getDb($iFetchMode = zsDb::FETCH_MODE_NUM,$eCommerceConn = false)
    {
        if (defined('OXID_PHP_UNIT')) {
        ...
        if (self::$_oDB === null || self::$_blNewConn) {

            $oInst = self::getInstance();
            //setting configuration on the first call
            if($eCommerceConn)
                $oInst->setConfigECommerce(oxRegistry::get("oxConfigFile"));
            else
